...
def zap_started(zap, target):
    ...
    if current_user == 'root':
        ...
        base_dir = f'/home/{current_user}'
    os.system(f"cp -R /zap/reports/traditional-json/report.json {base_dir}/.ZAP/reports/traditional-json/report.json")
    ...
    try:
        ...
        if config.disable_rules:
            ...
            ascan_disabled_rules = set(config.disable_rules).intersection(
                set(_all_active_scanner_rules(zap, Constants.ZAP_ACTIVE_SCAN_POLICY_NAME))
            )
...
def _all_active_scanner_rules(zap, policy_name) -> List[str]:
    return [scanner['id'] for scanner in zap.ascan.scanners(policy_name)]


def _all_passive_scanner_rules(zap) -> List[str]:
    return [scanner['id'] for scanner in zap.pscan.scanners]
